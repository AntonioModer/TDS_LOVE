...
extern number renderTargetSize, lightRadius;
...
vec4 effect(vec4 color, Image texture, vec2 texCoord, vec2 screenCoord) {
	...
	// apply a ...-pixel bias
	dist -= 5.0 / (renderTargetSize / ( 500 / lightRadius));							// 500 - это light radius standart; @todo -? для каждого света свое значение
